...
##################################################################
#
# Processing Script for Four-Year-Grad-Rates-by-Race-Ethnicity
...
four_yr_gr <- merge(four_yr_gr_grad, four_yr_gr_nongrad, by = c("District", "Race/Ethnicity", "Total Cohort Count", "Year"))
...
backfill_years <- expand.grid(
  ...
  `Race/Ethnicity` = unique(four_yr_gr$`Race/Ethnicity`),
  `Year` = years
)
...
complete_four_yr_gr <- complete_four_yr_gr[!with(complete_four_yr_gr, is.na(complete_four_yr_gr$`Race/Ethnicity`)),]
...
test <- complete_four_yr_gr_long[,c("District", "Year", "Race/Ethnicity", "Variable")]
test2<-test[duplicated(test), ]

#Write CSV
write.table(
  complete_four_yr_gr_long,
  file.path(path_to_top_level, "data", "four_year_grad_rate_by_race_ethnicity_2011-2019.csv"),
  sep = ",",
  row.names = F
)
